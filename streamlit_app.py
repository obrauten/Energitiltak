...
def stepper(label: str, key: str, initial: int, step: int, min_val: int, max_val: int):
    """±-kontroll i tre kolonner. Returnerer gjeldende verdi"""
...
def besparelse_sfp(qv_m3_h: float, SFP_old: float, SFP_new: float, driftstimer: float) -> float:
    ...
    qv_m3_s = qv_m3_h / 3600.0
    return dSFP * qv_m3_s * driftstimer
...
def besparelse_tempreduksjon(Q_space_kWh_year: float, delta_T_C: float) -> float:
    ...
    Q_space_kWh_year = årlig netto romoppvarming (kWh/år) i dag.
    """
...
def besparelse_nattsenking(Q_space_kWh_year: float, setback_C: float, timer_per_dogn: float) -> float:
    """
    Forenkling: samme 5 %/°C, men skalert med andel av døgnet med senking.
    """
    duty = max(min(timer_per_dogn, 24.0), 0.0) / 24.0
    return max(Q_space_kWh_year * 0.05 * max(setback_C, 0.0) * duty, 0.0)

# Hjelpetabell for belysning i næringsbygg (typiske gamle → moderne LED)
# Effekt per armatur (W). Timer settes i UI.
LUMINAIRE_MAP = [
    {"navn": "T8 2×58 W (armatur m/konvensjonell forkobling)", "gammel_W": 2*58 + 10, "LED_W": 2*30},
...
with st.sidebar:
    ...
    st.caption("Utslippsfaktor oppgis i **gram CO₂/kWh**. Resultatene vises i **kg CO₂/år**.")
...
with tabs[4]:
    ...
    deltaT = st.slider("Reduksjon i settpunkt (°C)", 0.0, 5.0, 1.0, 0.5)
...
with tabs[5]:
    ...
    Q_space = st.number_input("Årlig netto romoppvarming (kWh/år)", min_value=1000, max_value=50_000_000, value=600_000, step=10_000, key="night_Q")
    setback = st.slider("Senking (°C) i senketid", 0.0, 6.0, 2.0, 0.5)
    hours = st.slider("Timer per døgn med senking", 0, 24, 8, 1)
    kWh_calc = besparelse_nattsenking(Q_space, setback, hours)
    init_kWh = int(round(kWh_calc / 1000.0)) * 1000
...
with tabs[6]:
    ...
    valg = st.selectbox("Velg eksisterende armaturtype", navn_liste, index=0)
    data = next(d for d in LUMINAIRE_MAP if d["navn"] == valg)

    colA, colB, colC = st.columns(3)
    with colA:
        ant = st.number_input("Antall armaturer (stk)", min_value=0, max_value=1_000_000, value=200, step=10)
    with colB:
        timer = st.number_input("Driftstimer/år", min_value=100, max_value=HOURS_YEAR, value=3000, step=100)
    with colC:
        pass
    ...
    if data["gammel_W"] is None:
        col1, col2 = st.columns(2)
        with col1:
            W_old = st.number_input("Effekt pr gammel armatur (W)", min_value=1, max_value=2000, value=200, step=5)
        with col2:
            W_led = st.number_input("Effekt pr LED-armatur (W)", min_value=1, max_value=2000, value=100, step=5)
    else:
        W_old = st.number_input("Effekt pr gammel armatur (W)", min_value=1, max_value=2000, value=int(data["gammel_W"]), step=1)
        W_led = st.number_input("Effekt pr LED-armatur (W)", min_value=1, max_value=2000, value=int(data["LED_W"]), step=1)

    kWh_calc = besparelse_belysning(ant, W_old, W_led, timer)
    init_kWh = int(round(kWh_calc / 1000.0)) * 1000
    kWh = stepper("Energi spart (kWh/år)", key="lys_kwh", initial=init_kWh, step=1000, min_val=0, max_val=100_000_000)

    if st.button("Beregn besparelse", key="btn_lys"):
        kr, kg = nok_og_co2(kWh, pris, utslipp_g)
        st.success(f"Energi spart: **{fmt_int(kWh)} kWh/år** (teknisk: {fmt_int(kWh_calc)} kWh/år)")
        st.info(f"Kostnadsbesparelse: **{fmt_int(kr)} kr/år**  |  CO₂-reduksjon: **{fmt_int(kg)} kg/år**")

st.divider()
st.caption("Tips: Juster ‘Energi spart (kWh/år)’ med ± i 1000-steg for å tilpasse grove beregninger til lokale erfaringstall. Areal i etterisolering justeres i trinn på 10 m².")
